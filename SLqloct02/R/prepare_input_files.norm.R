...
#    state.vector <- c( rep(seq(-2,2,length.out = 6),6), 
#                       rep(seq(-2,2,length.out = 6),each=6) )
...
#    state.vector<-state.vector[,-c(1,6,31,36)] ## remove some columns    
     state.vector <- simulateMvMatrix(nreaz,
                        ...
                        param.list = list(    xloc = list(mean=25.0, sd=20.0, min=-5.0,max=55.0),
                                               yloc = list(mean=-18.0,sd=10.0,min=-33.0, max=-3.0),
	...
	if (state.vector[3,ireaz] >  30)  {state.vector[3,ireaz]=state.vector0[3,ireaz]} 
	if (state.vector[4,ireaz] <  0 )  {state.vector[4,ireaz]=state.vector0[4,ireaz]} 
	if (state.vector[4,ireaz] >  30)  {state.vector[4,ireaz]=state.vector0[4,ireaz]} 
    }
...
for (ireaz in 1:nreaz)
{
    ...
    pflotranin[flux_line+9] = paste(formatC(state.vector[4,ireaz], format = "e", digits = 5),"  ", formatC(state.vector[3,ireaz], format = "e", digits = 5))    
