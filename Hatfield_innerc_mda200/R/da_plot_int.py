...
for ifile in range(len(filenames0)):
  ...
  if (np.max(np.abs(data.get_array('y'))) < 1e-3):
    plt.plot(data.get_array('x'),data.get_array('y'),label=ifile)
  #print(filenames0[ifile])
  #print(np.max(np.abs(data.get_array('y'))))

plt.text(0.2, 0.1, '64 realizations',horizontalalignment='center', 
