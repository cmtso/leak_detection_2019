...
#    state.vector <- c( rep(seq(-10,10,length.out = 6),6), 
#                       rep(seq(-10,10,length.out = 6),each=6) )

#    state.vector <- matrix(state.vector,nrow=2,byrow = TRUE)
#    state.vector<-state.vector[,-c(1,6,31,36)] ## remove some columns    
#    rownames(state.vector)<-c("xloc","yloc")
    
    state.vector <- simulateMvMatrix(nreaz,
                        distributions = c( xloc = "unif",yloc="unif",q="norm",qt0="norm" ),
                        param.list = list( xloc = list(min=-10.0,max=10.0),
                                           yloc = list(min=-10.0, max=10.0),
					      q = list(mean=10.0, sd=3.0)
				            qt0 = list(mean=40.0, sd=10.0)	),
    ...
    {
     	if (state.vector[1,ireaz] < -10)  {state.vector[1,ireaz]=state.vector0[1,ireaz]} 
     	if (state.vector[1,ireaz] >  10)  {state.vector[1,ireaz]=state.vector0[1,ireaz]}      		
        if (state.vector[2,ireaz] < -10)  {state.vector[2,ireaz]=state.vector0[2,ireaz]}      	
	if (state.vector[2,ireaz] >  10)  {state.vector[2,ireaz]=state.vector0[2,ireaz]} 
	if (state.vector[3,ireaz] <   0)  {state.vector[3,ireaz]=state.vector0[3,ireaz]} 
	if (state.vector[4,ireaz] <   0)  {state.vector[4,ireaz]=state.vector0[4,ireaz]} 
	if (state.vector[4,ireaz] >  85)  {state.vector[4,ireaz]=state.vector0[4,ireaz]} 
    }
...
write.table(t(as.matrix(c(iter,apply(state.vector,1,mean)))),file="results/summary.mean.txt",sep="\t",col.names = F,row.names = F,append = T)
write.table(t(as.matrix(c(iter,apply(state.vector,1,sd)))),file="results/summary.sd.txt",sep="\t",col.names = F,row.names = F,append = T)

## write to pflotran input
pflotranin = readLines(paste("template/",mc_name,".in",sep=''))
loc_line = grep("REGION leak_loc1",pflotranin)[1]
qt0_line = grep("RATE LIST",pflotranin)[1]

for (ireaz in 1:nreaz)
{
    pflotranin[loc_line+1] = paste("COORDINATE","  ",formatC(state.vector[1,ireaz], format = "e", digits = 4), formatC(state.vector[2,ireaz], format = "e", digits = 4), "  -0.5") 
    pflotranin[qt0_line+4] = paste("      ",formatC(state.vector[3,ireaz], format = "e", digits = 4),
				"     ", formatC(state.vector[4,ireaz], format = "e", digits = 4) ) 
