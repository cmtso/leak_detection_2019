...
###====Clear old results
rm -r pflotran_mc #pflotran_mc0 pflotran_mc2
mkdir pflotran_mc #pflotran_mc0 pflotran_mc2
...
do
    ...
    R CMD BATCH --slave R/morris.R

	echo "finish creating input files"
  #	R CMD BATCH --slave "--args mode=1"  R/lookup_rename.R

    shell/mc.fuji.sh 
#	R CMD BATCH --slave  "--args mode=0" R/lookup_rename.R
...
#	R CMD BATCH --slave "--args iter=$iter alpha=$niter" R/alpha.update.R
